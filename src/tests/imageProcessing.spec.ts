...
describe('imageProcessing utility', () => {
    describe('Environment Setup', () => {
        it('should have the test image windowsxp.jpg in the input directory', () => {
            expect(
                filesystem.fileExists(REAL_IMAGE, filesystem.INPUT_PATH),
            ).toBeTrue();
        });
    });
    ...
    describe('extractImageQueryParams', () => {
        it('All parameters provided', () => {
            const req = {
                query: { filename: 'cat.jpg', width: '200', height: '150' },
    ...
    describe('getOutputImageFilename', () => {
        it('No dimensions provided', () => {
            expect(
                imageProcessing.getOutputImageFilename(
                    'cat.jpg',
                    undefined,
                    undefined,
                ),
        ...
        it('Both dimensions provided', () => {
            expect(
                imageProcessing.getOutputImageFilename('cat.jpg', 200, 150),
        ...
        it('PNG extension', () => {
            expect(
                imageProcessing
                    .getOutputImageFilename('photo.png', 100, 100)
                    .endsWith('.png'),
            ).toBeTrue();
        });
        it('GIF extension', () => {
            expect(
                imageProcessing
                    .getOutputImageFilename('photo.gif', 100, 100)
                    .endsWith('.gif'),
            ).toBeTrue();
        });
        it('JPEG extension', () => {
            expect(
                imageProcessing
                    .getOutputImageFilename('photo.jpeg', 100, 100)
                    .endsWith('.jpeg'),
            ).toBeTrue();
        });
    });
    ...
    describe('Image Processing Functions', () => {
        ...
        it('processImage should create a resized image', async () => {
            await imageProcessing.processImage(
                REAL_IMAGE,
                TEST_WIDTH,
                TEST_HEIGHT,
            );
            expect(
                filesystem.fileExists(OUTPUT_FILE, filesystem.OUTPUT_PATH),
            ).toBeTrue();
        });

        it('getMetadataFromFilename should return image metadata', async () => {
            const metadata = await imageProcessing.getMetadataFromFilename(
                REAL_IMAGE,
